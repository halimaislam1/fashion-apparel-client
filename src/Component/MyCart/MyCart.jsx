...
const MyCart = () => {
  ...
  const userProducts = productCart.filter(
    (products) => products.userEmail === userEmail
  );
  ...
  const handleDelete = (_id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://fashion-apparel-server-bice.vercel.app/cart/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  return (
    <div >
      <div >
         <div className={userProducts.length !== 0 && "block"} >
          ...
          {userProducts?.map((cart) => (
             ...
             >
               ...
               <div className="card-body  ">
                 <h2 className="card-title dark:text-white font-bold">
                   {cart.name}
                 </h2>
                 ...
                 <div className="flex items-center justify-between">
                   ...
                   <div>
                     <button
                       ...
                       className="p-2 bg-amber-500 text-white font-bold rounded"
                     >
                       Delete
                     </button>
                   </div>
    ...
    <div className={userProducts.length== 0 ? 'block' : 'hidden'}>
          <div className=" text-center pb-20 text-amber-500">
          <h1 className=" text-3xl">No product found</h1>
           <p className="my-3">Please add product </p>
          </div>
      </div>
    </div>
